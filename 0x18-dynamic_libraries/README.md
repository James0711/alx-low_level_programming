...
This project focuses on using dynamic libraries in C. A dynamic library also known as shared library/ Dynamic Library is a library that is loaded dynamically at runtime for each application that requires it. Dynamic Linking doesn’t require the code to be copied, it is done by just placing name of the library in the binary file. The actual linking happens when the program is run, when both the binary file and the library are in memory.

Here we learned about how to create and use a dynamic library with $LD_LIBRARY_PATH, nm, ldd, and ldconfig.
